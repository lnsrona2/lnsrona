...
C1::AST::DeclRefExpr::DeclRefExpr(DeclContext* lookup_context, const std::string &name)
: m_Name(name), m_RefContext(lookup_context)
{
	ResoloveReference();
}
...
void C1::AST::DeclRefExpr::ResoloveReference()
{
...
bool C1::AST::IndexExpr::HasSideEffect() const
{
	throw std::logic_error("The method or operation is not implemented.");
}
...
: m_HostExpr(host), m_MemberName(member_name), m_Operator(op)
{
	...
	{
		throw std::logic_error("Left operand of member operator must have record types.");
	}
	else
	{
		...
		if (!decl)
		{
			...
			throw std::runtime_error("Right operand is not an member of left operand.");
		}
	}
}
...
{
	return QualType(MakePointer(base_type),QualifierMask());
}
...
{
	os << *m_Base << "[" << *m_SizeExpr << "]";
}

C1::AST::QualType C1::AST::ArrayDeclarator::DecorateType(QualType base_type)
{
	auto decoratedType = MakeArray(base_type, SizeExpr()->Evaluate().Int);
...
C1::AST::QualType C1::AST::FunctionalDeclarator::DecorateType(QualType base_type)
{
	...
	auto decoratedType = Context->NewFunctionType(base_type,std::move(parameterTypes));
	return decoratedType;
}
...
{
	return SubExpr()->ReturnType().As<PointerType>()->Base();
}
...
C1::AST::SizeofTypeExpr::SizeofTypeExpr(TypeExpr* type_expr) : m_TypeExpr(type_expr)
{
...
{
	os << "i_cast<" << m_TargetType->ToString() << ">(" << *m_SourceExpr << ")";
}
...
C1::AST::IterationStmt::IterationStmt(Stmt* action /*= nullptr*/, Expr* condtion /*= nullptr*/) : m_Action(action), m_Condition(condtion)
{
